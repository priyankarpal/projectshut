          ...
          <dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-10 lg:max-w-none lg:grid-cols-2 lg:gap-y-16">
            {features.map((feature, i) => (
              <motion.div
                ...
                key={feature.name} className=" p-5 hover:outline outline-1 outline-gray-600 rounded">
                <div className="relative pl-16">
                  ...
                  <motion.div
                    ...
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.5, type: "spring", stiffness: 110 , delay: (i * 0.2) + 0.1 }}
                  >{feature.name}</motion.div>
                </div>
                <motion.div
                  initial={{ opacity: 0, x: -40 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.5, type: "spring", stiffness: 110, delay: (i * 0.2) + 0.2 }}
